      ...
      >
        {aiInsights && (
          <div className="mb-6 bg-purple-50 border border-purple-200 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-700 mb-3">{aiInsights.overallAnalysis}</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {aiInsights.patterns?.length > 0 && (
                <div className="bg-white p-3 rounded-lg">
                  <h4 className="text-sm font-medium text-blue-700 mb-1">Patterns Detected</h4>
                  <ul className="text-xs text-gray-600 space-y-1">{aiInsights.patterns.map((p, i) => <li key={i}>- {p}</li>)}</ul>
                </div>
              )}
              {aiInsights.burnoutWarnings?.length > 0 && (
                <div className="bg-white p-3 rounded-lg">
                  <h4 className="text-sm font-medium text-red-700 mb-1">Burnout Warnings</h4>
                  <ul className="text-xs text-gray-600 space-y-1">{aiInsights.burnoutWarnings.map((w, i) => <li key={i}>- {w}</li>)}</ul>
                </div>
              )}
              {aiInsights.coverageRisks?.length > 0 && (
                <div className="bg-white p-3 rounded-lg">
                  <h4 className="text-sm font-medium text-amber-700 mb-1">Coverage Risks</h4>
                  <ul className="text-xs text-gray-600 space-y-1">{aiInsights.coverageRisks.map((r, i) => <li key={i}>- {r}</li>)}</ul>
                </div>
              )}
              {aiInsights.staffingRecommendations?.length > 0 && (
                <div className="bg-white p-3 rounded-lg">
                  ...
                  <ul className="text-xs text-gray-600 space-y-1">{aiInsights.staffingRecommendations.map((r, i) => <li key={i}>- {r}</li>)}</ul>
                </div>
              )}
            </div>
            {aiInsights.forecast?.length > 0 && (
              <div className="mt-3">
                <h4 className="text-sm font-medium text-gray-700 mb-2">Leave Forecast</h4>
                <div className="flex gap-2 overflow-x-auto">
                  {aiInsights.forecast.map((f, i) => (
                    <div key={i} className="bg-white p-2 rounded-lg min-w-[120px] text-center">
                      <p className="text-xs font-medium text-gray-800">{f.month}</p>
