  ...
  return (
    <FeatureGate feature="COMPLIANCE_REMINDERS">
      ...
      <IdentityBand
        ...
        subtitle="Retention reminders and what they are waiting on"
      />
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              <div key={s.label} className="bg-card dark:bg-card rounded-lg shadow p-4 text-center">
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {reminders.map((reminder) => (
                <div key={reminder.id} className="bg-card dark:bg-card rounded-lg shadow p-5">
                  <div className="flex items-start justify-between">
                    <div className="flex items-start gap-3">
                      {statusIcon(reminder.status)}
                      <div>
                        <h4 className="font-medium text-foreground dark:text-white">{reminder.title}</h4>
                        {reminder.description && (
                          <p className="text-sm text-muted-foreground mt-1">{reminder.description}</p>
                        )}
                        <div className="flex gap-3 mt-2 text-xs text-muted-foreground">
                          <span className="px-2 py-0.5 rounded-full bg-muted dark:bg-muted">{typeLabel(reminder.reminderType)}</span>
