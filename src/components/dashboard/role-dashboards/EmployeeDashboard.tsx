...
// Employees get the same job-search dashboard shape as Applicants — Browse
// Jobs, My Applications, Interview Schedule, and My Offers are now real
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        // endpoints — no self-service role has ever had access, including
        // Applicant. getMyDashboardData resolves the caller's own applicant
        // record and walks their applications/interviews instead.
        ...
        <div className="lg:col-span-2 space-y-6 min-w-0">
          ...
          <div className="w-full overflow-hidden">
            <DashboardWidget
              ...
              subtitle="Track the progress of your internal or external applications"
              refreshable={true}
              size="large"
            >
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div key={application.id} className="flex items-center justify-between p-4 border border-border rounded-control hover:bg-muted">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button
                          ...
                          className="text-gold-600 hover:text-gold-800 text-sm font-medium rounded-full"
                        >
                          View Details
                        </button>
                      </div>
          ...
          <div className="w-full overflow-hidden">
            ...
            >
              <div className="grid grid-cols-1 gap-2">
                {[
                  { label: 'Internal Jobs', color: 'bg-gold-500 text-violet-950', href: '/internal/jobs' },
                  { label: 'Browse Jobs', color: 'bg-primary text-white', href: '/candidate/jobs' },
                  { label: 'My Applications', color: 'bg-green-600 text-white', href: '/candidate/applications' },
                  { label: 'Update Profile', color: 'bg-orange-600 text-white', href: '/candidate/profile' },
                ].map((action) => (
                  <button
                    key={action.label}
