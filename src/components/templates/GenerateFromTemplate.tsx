...
const GenerateFromTemplate: React.FC<GenerateFromTemplateProps> = ({
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setInitError('Failed to load templates and requisitions');
      } finally {
        ...
        <div className="space-y-5">
          ...
          {generatedDraft.intro && (
            <div
              ...
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(generatedDraft.intro) }}
            />
          )}
          ...
          {generatedDraft.responsibilities && (
            <div
              className="prose prose-sm max-w-none text-foreground"
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(generatedDraft.responsibilities) }}
            />
          )}
          ...
          {generatedDraft.requirements && (
            <div
              className="prose prose-sm max-w-none text-foreground"
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(generatedDraft.requirements) }}
            />
          )}
          ...
          {generatedDraft.benefits && (
            <div
              className="prose prose-sm max-w-none text-foreground"
              dangerouslySetInnerHTML={{ __html: sanitizeHtml(generatedDraft.benefits) }}
            />
          )}
      ...
      <div className={`text-center py-12 ${className}`}>
        ...
        <div className="flex justify-center space-x-3">
          ...
          <button
            ...
            className="inline-flex items-center px-4 py-2 border-2 border-cta text-sm font-medium rounded-full text-cta-on-surface bg-transparent hover:bg-cta hover:text-cta-foreground uppercase tracking-wider transition-colors"
          >
            View Draft
          </button>
        </div>
  ...
  return (
    <div className={className}>
      ...
      {/* Only render main content when data has loaded */}
          ...
          <div>
              ...
              <div className="space-y-5">
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="inline-flex items-center px-4 py-2 border-2 border-cta text-sm font-medium rounded-full text-cta-on-surface bg-transparent hover:bg-cta hover:text-cta-foreground disabled:opacity-50 disabled:cursor-not-allowed uppercase tracking-wider transition-colors"
                  >
                    Next: Customize Data
                  </button>
                </div>
            ...
            {step === 'customize' && (
              <div className="space-y-5">
                ...
                {selectedRequisition && (
                  <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-card">
                    <h4 className="text-sm font-medium text-green-900 dark:text-green-300 mb-2">From Requisition: {selectedRequisition.jobTitle}</h4>
                    <div className="grid grid-cols-2 gap-3 text-xs">
                      <div>
                        <span className="text-green-700 dark:text-green-400">Job Title:</span> {selectedRequisition.jobTitle}
                      </div>
                      <div>
                        <span className="text-green-700 dark:text-green-400">Department:</span> {selectedRequisition.department}
                      </div>
                      <div>
                        <span className="text-green-700 dark:text-green-400">Location:</span> {selectedRequisition.location}
                      </div>
                      <div>
                        <span className="text-green-700 dark:text-green-400">Employment Type:</span> {getEnumLabel('employmentType', selectedRequisition.employmentType)}
                      </div>
                    </div>
                  </div>
                )}
