      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="col-span-2">
          ...
          <input type="text" value={candidateExperience} onChange={e => setCandidateExperience(e.target.value)}
            className="w-full text-sm p-2 border border-gray-300 rounded-control" placeholder="e.g. 5 years in backend development" />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="flex gap-2">
          <input type="text" value={skillInput} onChange={e => setSkillInput(e.target.value)}
            ...
            className="flex-1 text-sm p-2 border border-gray-300 rounded-control" placeholder="Add a skill" />
      ...
      {result && result.questions && result.questions.length > 0 && (
        <div className="space-y-2">
          {result.questions.map((q, i) => (
            <div key={i} className={`border rounded-control p-3 cursor-pointer transition-colors ${selected.has(i) ? 'border-violet-300 bg-gold-50' : 'border-gray-200 bg-gray-50'}`}
