...
/**
 * DynamoDB item for the Applicant entity.
 *
 * Table keys:
 *   PK:  TENANT#{tenantId}
 *   SK:  APPLICANT#{id}
 *
 * GSI1 (created date sort for recent queries):
 *   GSI1PK: APPLICANT_CREATED#{tenantId}
 *   GSI1SK: APPLICANT#{createdAt}
 *
 * GSI2 (FK lookup — by userId):
 *   GSI2PK: APPLICANT_USER#{userId}
 *   GSI2SK: APPLICANT#{id}
 *
 * GSI4 (unique constraint — email per tenant):
 *   GSI4PK: APPLICANT_EMAIL#{tenantId}#{email}
 *   GSI4SK: APPLICANT#{id}
 */
@DynamoDbBean
public class ApplicantItem {
    ...
    // ── GSI1: Created date index (for recent queries) ─────────────────────
